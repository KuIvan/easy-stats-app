class EasyStatsAppSchema < GraphQL::Schema
  ...
  rescue_from(ActiveRecord::RecordInvalid) do |exception, obj, args, ctx, field|
    raise GraphQL::ExecutionError, "Invalid attributes for #{exception.record.class}:"\
          " #{exception.record.errors.full_messages.join(', ')}"
  end

  rescue_from(ActiveRecord::RecordNotFound) do |exception, obj, args, ctx, field|
