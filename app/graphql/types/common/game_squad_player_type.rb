module Types
  module Common
    class GameSquadPlayerType < Types::BaseObject
      ...
      def assist_count
        object.game.actions.where('scope = 1 AND initiator_id = :game_squad_player', game_squad_player: object.id).count
      end

      def yellow_card_count
        object.game.actions.where('scope = 14 AND initiator_id = :game_squad_player', game_squad_player: object.id).count
      end

      def red_card_count
        object.game.actions.where('scope = 15 AND initiator_id = :game_squad_player', game_squad_player: object.id).count
      end
